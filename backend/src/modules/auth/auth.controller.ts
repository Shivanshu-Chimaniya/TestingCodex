...
export async function login(req: Request, res: Response) {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ accessToken, refreshToken, user: { id: user.id, username: user.username, email: user.email } });
  } catch (error) {
...
export function refresh(req: Request, res: Response) {
  ...
  try {
    const { accessToken, refreshToken } = authService.refreshAccessToken(parsed.data.refreshToken);
